...
function start() 
local t = gg.prompt({'Золотой', 'Обычный', 'Серебряный', 'Выход'}, nil, {'checkbox', 'checkbox', 'checkbox', 'checkbox'})
if t == nil then
    gg.sleep(300)
else
    if t[1] then
...
function hh() 
gg.toast("Проверка выбора...")
gg.sleep(900)

if chrome ~= nil then
gg.alert("Проверка пройдена, продолжаю.")
ho() 
else
gg.alert("Проверка не пройдена, выхожу.")
main()
end
end

function ho() 
gg.alert("Найди и подними Reflection")
gg.sleep(3000)
gg.setRanges(gg.REGION_ANONYMOUS)
...
gg.alert("⬆️ Подними ⬆️")
gg.sleep(3000)
gg.refineNumber("1", gg.TYPE_FLOAT) 
gg.sleep(3000)
gg.getResults(9999)
gg.editAll(chrome, gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("😎Всё готово👍")
gg.alert("Менять цвет можно в main и specular")
end 

local r = gg.prompt({'Уже делал(а) ', 'Не делал(а) ', 'Выход'}, nil, {'checkbox', 'checkbox', 'checkbox'})
if r == nil then
    gg.alert("Выхожу.")
main()
else
    ...
    if r[2] then
    gg.sleep(300)
        gg.alert("Зайдите во внешний вид авто и выберите Reflection поставьте его на белый цвет 'саммый вверх' у тебя 3 секунды '") 
        gg.sleep(1000)
        gg.toast("1")
        gg.sleep(1000)
        gg.toast("2")
